...
public class CommonRenderingMethods {
    ...
    public static void renderingPlacedCards(@NotNull BlockEntity tileEntityIn,
                                            PoseStack poseStack,
        ...
        for (int i = Math.max(0, cardsPlacedNum - RENDERING_CARD_NUM); i < cardsPlacedNum; i++) {
            double deltaX = tableNbt.getDouble(i + "x") - tileEntityIn.getBlockPos()
                                                                      .getX();
            double deltaZ = tableNbt.getDouble(i + "z") - tileEntityIn.getBlockPos()
                                                                      .getZ();
            float rotation = tableNbt.getFloat(i + "r");
            int id = tableNbt.getInt(Integer.toString(i));
            ItemStack stack = ItemHandler.mino_card_shapes.get(id)
                                                          .get()
                                                          .getDefaultInstance();
            if (i == cardsPlacedNum - 1) {
                stack.enchant(Enchantments.ALL_DAMAGE_PROTECTION, 1);
                if (random.nextFloat() > 0.99f) {
                    double x = tableNbt.getDouble(i + "x");
                    double z = tableNbt.getDouble(i + "z");
                    double y = tileEntityIn.getBlockPos()
                                           .getY() + 1.5f;
                    Objects.requireNonNull(tileEntityIn.getLevel())
    ...
    public static void renderingDeck(@NotNull BlockEntity tileEntityIn,
                                     PoseStack poseStack,
                                     MultiBufferSource bufferSource,
                                     ItemRenderer itemRenderer,
                                     int combinedLight,
    ...
    public static void renderingDepositDeck(@NotNull BlockEntity tileEntityIn,
                                            PoseStack poseStack,
                                            MultiBufferSource bufferSource,
                                            ItemRenderer itemRenderer,
                                            int combinedLight,
